...
void main() {
    ...
    // Khởi tạo UART theo thứ tự: UART1 trước (BMS), UART2 sau (Communication)
    UART1_Init(9600);  // BMS sử dụng UART1
    UART2_Init(9600);  // Communication sử dụng UART2
    _UART2_Init();     // Khởi tạo UART2 custom

    DebugUART_Init();
    _MotorDC_Init(&motorDC, 2.5, 0.5, 1.0, 0);
    _MotorDC_SetSafeDistance(&motorDC, 40);
    _Lifter_Init(&lifter, 1.0, 0.5, 0.1, 30);
    BMS_Init();        // BMS_Init() sẽ khởi tạo UART1
    Lms_Init();
